<?php
include_once '/home/rafasantoni.heliohost.us/httpdocs/EMkt/includes/config.php';
...
if (!isset($_GET['regiao']) || empty($_GET["regiao"]) || !is_numeric($_GET["regiao"])){
	echo '<h2>Erro ao processar</h2>N?o foi possivel completar a opera??o. <BR />Regi&atilde;o n?o definida!'; exit();
...
$rsRegiao = mysqli_fetch_object($Banco->resultado);
...
if (!isset($_GET['lvl']) || empty($_GET["lvl"]) || !is_numeric($_GET["lvl"])){
	echo '<h2>Erro ao processar</h2>N?o foi possivel completar a opera??o. <BR />N&iacute;vel n&atilde;o definido!'; exit();
	//echo '<script>history.back();</script>';
}
...
//if(mysql_num_rows($qr) <= 0){
	echo 'N?o foi possivel completar a opera??o.'; exit();
}
